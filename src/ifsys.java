    ...
    implements MouseListener, MouseMotionListener, KeyListener, FocusListener
{
    ...
    public void addPoint(){
        shape.pts[pointsInUse].x = mousex;
        shape.pts[pointsInUse].y = mousey;
        shape.pts[pointsInUse].scale = 0.5D;
        shape.pts[pointsInUse].rotation = 0.0D;
        pointsInUse++;
        updateCenter();
        clearframe();
        gamefunc();
    ...
    public void deletePoint(){
        ...
        shape.pts[pointsInUse].x = 0.0D;
        shape.pts[pointsInUse].y = 0.0D;
    ...
    public void updateCenter(){
        ...
        if(pointsInUse != 0){
            ...
            centerx = shape.pts[0].x;
            centery = shape.pts[0].y;
        }
    }
    ...
    public void paint(Graphics gr){
        ...
        if(!infoHidden){
            ...
            rg.drawString("Rotation (kl op 90): " + String.valueOf((double)(int)((((shape.pts[pointselected].rotation / pi) * 180D + 36000000D) % 360D) * 1000D) / 1000D), 5, 75);
    ...
    public void gamefunc(){
        for(int a = 0; a < pointsInUse; a++){
            shape.pts[a].degrees = Math.atan2(shape.pts[a].x - centerx, shape.pts[a].y - centery);
            shape.pts[a].radius = distance2(shape.pts[a].x - centerx, shape.pts[a].y - centery);
        }
        ...
        if(pointsInUse != 0){
            for(int a = 0; a < sampletotal; a++){
                ...
                for(int d = 1; d < iterations; d++){
                    ...
                    dx += Math.cos((pi2 - shape.pts[c].degrees) + extra) * shape.pts[c].radius * e;
                    dy += Math.sin((pi2 - shape.pts[c].degrees) + extra) * shape.pts[c].radius * e;
                }
    ...
    public void mouseDragged(MouseEvent e){
        if(mousemode == 1){ //left click to move a point
            ...
            shape.pts[pointselected].rotation = pi * 2 - (Math.atan2(e.getX() - shape.pts[pointselected].x, e.getY() - shape.pts[pointselected].y)- startDragAngle);
